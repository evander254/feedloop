...
export default function Dashboard() {
  ...
  if (checking) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-[#ecfdf5] via-[#d1fae5] to-[#f0fdf4]">
        <div className="flex flex-col items-center gap-3">
          <div className="size-10 animate-spin rounded-full border-[3px] border-emerald-400 border-t-transparent" />
          <p className="text-xs font-semibold text-emerald-700/50">Loading FeedLoop...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative min-h-screen bg-gradient-to-br from-[#ecfdf5] via-[#d1fae5] to-[#f0fdf4] text-slate-800 font-sans flex flex-col overflow-x-hidden">

      {/* Top Navbar */}
      <header className="relative z-10 flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => navigate("/")}>
          <img src={logoSrc} alt="FeedLoop Logo" className="h-8 w-auto object-contain" />
          <span className="text-lg font-black tracking-tight text-emerald-900">FeedLoop</span>
        </div>
        ...
        <div className="relative" ref={profileRef}>
          <button
            ...
            className="flex size-9 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-teal-600 text-xs font-bold text-white shadow-md shadow-emerald-500/20 ring-2 ring-white/50 hover:scale-105 transition duration-200"
          >
            {userInitials}
          </button>

          {profileOpen && (
            <div className="absolute right-0 top-full z-50 mt-2 w-56 overflow-hidden rounded-2xl border border-white/30 bg-white/50 backdrop-blur-xl p-1.5 shadow-xl shadow-emerald-900/10">
              <div className="border-b border-white/20 px-4 py-3">
                <p className="text-sm font-bold text-emerald-900 truncate">{userName}</p>
                <p className="text-xs text-emerald-700/50 truncate">{userEmail}</p>
              </div>
              <div className="p-1">
                <button
                  type="button"
                  onClick={() => { setProfileOpen(false); navigate("/profile"); }}
                  className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left text-xs font-semibold text-emerald-800/70 hover:bg-white/40 transition"
                >
                ...
                <button
                  ...
                  className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left text-xs font-semibold text-red-500 hover:bg-red-50/40 transition"
                >
                  <LogOut size={14} />
                  Sign Out
                </button>
      ...
      <main className="relative z-10 flex-1 flex flex-col justify-start items-center max-w-6xl w-full mx-auto px-4 mt-6">
        ...
        <section className="text-center mb-8 max-w-2xl">
          <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-emerald-950 mb-2">
            Welcome to FeedLoop, {userName}!
          </h1>
          <p className="text-sm sm:text-base text-emerald-700/60 font-medium mb-8">
            Collect better data and make better decisions.
          </p>
          <p className="text-xs sm:text-sm text-emerald-600/40 font-semibold mb-2">
        ...
        <section className="w-full bg-white/40 backdrop-blur-xl rounded-3xl p-6 shadow-sm border border-white/30 mb-16 z-10">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/20 pb-4 mb-6">
            <h2 className="text-lg font-bold text-emerald-950 flex items-center gap-2">
              My Files &amp; Workspace
              <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-700/60 border border-emerald-500/10">
                {counts.forms + counts.surveys + counts.polls} total
              </span>
            </h2>

            <div className="flex items-center gap-1 bg-white/30 backdrop-blur-sm p-1 rounded-xl border border-white/20">
              <button
                onClick={() => setActiveTab("forms")}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === "forms" ? "bg-white/60 text-emerald-900 shadow-sm backdrop-blur-sm" : "text-emerald-700/40 hover:text-emerald-800"}`}
              >
                Forms ({counts.forms || 0})
              </button>
              <button
                onClick={() => setActiveTab("drafts")}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === "drafts" ? "bg-white/60 text-emerald-900 shadow-sm backdrop-blur-sm" : "text-emerald-700/40 hover:text-emerald-800"}`}
              >
                Drafts ({drafts.length || 0})
              </button>
              <button
                onClick={() => setActiveTab("surveys")}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === "surveys" ? "bg-white/60 text-emerald-900 shadow-sm backdrop-blur-sm" : "text-emerald-700/40 hover:text-emerald-800"}`}
              >
                Surveys ({counts.surveys || 0})
              </button>
              <button
                onClick={() => setActiveTab("polls")}
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${activeTab === "polls" ? "bg-white/60 text-emerald-900 shadow-sm backdrop-blur-sm" : "text-emerald-700/40 hover:text-emerald-800"}`}
              >
                ...
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {recentForms.map((form) => (
                    <FileCard
                      ...
                      badgeColor={form.status === "published" ? "bg-emerald-500/10 text-emerald-700 border-emerald-500/15" : "bg-amber-500/10 text-amber-700 border-amber-500/15"}
      ...
      {/* Footer Branding */}
      <footer className="relative z-10 text-center py-6 text-xs text-emerald-700/30 border-t border-white/20 bg-white/20 backdrop-blur-md">
      ...
      <button
        ...
        className="fixed bottom-4 right-4 z-40 bg-gradient-to-r from-emerald-600 to-teal-600 text-white hover:from-emerald-500 hover:to-teal-500 px-4 py-2 rounded-full shadow-lg shadow-emerald-500/20 text-xs font-bold flex items-center gap-1.5 border border-white/20 hover:scale-105 active:scale-95 transition"
      >
      ...
      {feedbackOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-emerald-900/10 backdrop-blur-sm px-4">
          <div className="bg-white/50 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/30 max-w-md w-full p-6 relative">
            <button
              onClick={() => { setFeedbackOpen(false); setFeedbackSent(false); }}
              className="absolute top-4 right-4 text-emerald-600/40 hover:text-emerald-700 rounded-lg p-1 hover:bg-white/40 transition"
            >
              <X size={16} />
            </button>
            ...
            {feedbackSent ? (
              <div className="text-center py-6">
                <div className="size-12 bg-emerald-500/10 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4">
                  <ThumbsUp size={22} />
                </div>
                <h3 className="text-lg font-bold text-emerald-950">Thank You!</h3>
                <p className="text-xs text-emerald-700/50 mt-1">Your feedback helps us make FeedLoop better.</p>
              </div>
            ) : (
              <form onSubmit={handleSendFeedback} className="space-y-4">
                <div>
                  ...
                  <p className="text-[11px] text-emerald-600/40 mt-0.5">Let us know how we can improve your experience.</p>
                </div>
                ...
                <textarea
                  ...
                  className="w-full rounded-2xl border border-white/30 bg-white/30 backdrop-blur-sm px-4 py-3 text-xs outline-none focus:border-emerald-400 focus:bg-white/50 placeholder:text-emerald-700/20 resize-none text-emerald-900"
                />
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 bg-white/30 backdrop-blur-sm text-emerald-700 border border-white/30 rounded-xl py-2.5 text-xs font-bold hover:bg-white/50 transition"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 text-white rounded-xl py-2.5 text-xs font-bold hover:from-emerald-500 hover:to-teal-500 shadow-md shadow-emerald-500/20 transition"
                  >
                    Submit
                  </button>
                </div>
...
function FileCard({
  ...
  return (
    <div className="group flex flex-col justify-between p-4 rounded-2xl border border-white/30 bg-white/30 backdrop-blur-md hover:border-emerald-400/20 hover:bg-white/40 hover:shadow-md hover:shadow-emerald-500/5 transition duration-200">
      <div>
        ...
        <h4 className="text-sm font-bold text-emerald-950 line-clamp-1 group-hover:text-emerald-800">
          {title}
        </h4>
        <p className="text-xs text-emerald-700/40 line-clamp-2 mt-1 leading-normal">
          {desc || "No description provided."}
        </p>
      </div>

      <div className="mt-4 pt-3 border-t border-white/15 flex items-center justify-between">
        <span className="flex items-center gap-1 text-[10px] text-emerald-600/30">
          <Clock size={11} />
        ...
        <div className="flex gap-1.5 opacity-90 group-hover:opacity-100 transition">
          <button
            type="button"
            onClick={onView}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-white/30 bg-white/20 text-[10px] font-bold text-emerald-700/60 hover:bg-white/40 hover:border-emerald-400/20 transition"
          >
          ...
          <button
            type="button"
            onClick={onEdit}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-[10px] font-bold hover:from-emerald-500 hover:to-teal-500 shadow-sm shadow-emerald-500/20 transition"
          >
            Edit
          </button>
        </div>
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-10 text-center">
      <div className="size-12 bg-emerald-500/10 rounded-full flex items-center justify-center text-emerald-500/40 mb-3">
        <Sparkles size={20} />
      </div>
      <h3 className="text-sm font-bold text-emerald-800/60">No {label} Found</h3>
      <p className="text-xs text-emerald-600/30 max-w-xs mt-1">
        You haven't created any {label.toLowerCase()} in this workspace yet. Create one to get started.
      </p>
      <button
        type="button"
        onClick={() => navigate(actionPath)}
        className="mt-4 flex items-center gap-1 bg-gradient-to-r from-emerald-600 to-teal-600 text-white text-xs font-bold px-4 py-2 rounded-xl hover:from-emerald-500 hover:to-teal-500 hover:scale-[1.02] active:scale-[0.98] shadow-md shadow-emerald-500/20 transition"
      >
        <Plus size={13} />
        {btnText}
      </button>
