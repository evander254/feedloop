...
export default function SurveyResponses() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: responseData } = await supabase
        ...
        .select("id, submitted_at, survey_response_answers(field_id, answer)")
        .eq("survey_id", surveyId)
  ...
  const exportCsv = () => {
    ...
    const rows = responses.map((r) => {
      ...
      r.survey_response_answers?.forEach((a) => {
      ...
      <AppLayout>
        <div className="flex min-h-[60vh] items-center justify-center px-4">
          <div className="text-center">
            ...
            <Link to="/surveys" className="mt-4 inline-flex rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:-translate-y-px hover:bg-slate-800 hover:shadow-md">
              Back to Surveys
            </Link>
          </div>
        </div>
        ...
        <main className="flex-1 overflow-y-auto">
          <div className="space-y-4">
            <div className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <div>
                <h1 className="text-xl font-extrabold tracking-tight text-slate-900">{survey.title}</h1>
                {survey.description && (
